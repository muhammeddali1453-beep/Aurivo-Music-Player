...
void destroy_dsp(void *dsp) {
  delete static_cast<AurivoDSP::MasterDSP *>(dsp);
}

void process_dsp(void *dsp, float *buffer, int numFrames, int channels) {
  if (dsp)
    static_cast<AurivoDSP::MasterDSP *>(dsp)->processBuffer(buffer, numFrames,
                                                             channels);
}
...
void process_web_audio(void *dsp, float *buffer, int numFrames, int channels) {
  if (dsp)
    static_cast<AurivoDSP::MasterDSP *>(dsp)->processWebBuffer(
        buffer, numFrames, channels);
}

void set_eq_band(void *dsp, int band, float gain) {
  if (dsp)
    static_cast<AurivoDSP::MasterDSP *>(dsp)->setEQGain(band, gain);
}

void set_eq_bands(void *dsp, const float *gains, int numBands) {
  if (dsp)
    static_cast<AurivoDSP::MasterDSP *>(dsp)->setEQGains(gains, numBands);
}

void set_tone_params(void *dsp, float bass, float mid, float treble) {
  if (dsp)
    static_cast<AurivoDSP::MasterDSP *>(dsp)->setToneParams(bass, mid, treble);
}

void set_stereo_width(void *dsp, float width) {
  if (dsp)
    static_cast<AurivoDSP::MasterDSP *>(dsp)->setStereoWidth(width);
}

void set_master_toggle(void *dsp, int active) {
  if (dsp)
    static_cast<AurivoDSP::MasterDSP *>(dsp)->setMasterToggle(active != 0);
}

void set_dsp_enabled(void *dsp, int enabled) {
  if (dsp)
    static_cast<AurivoDSP::MasterDSP *>(dsp)->setDSPEnabled(enabled != 0);
}

void set_sample_rate(void *dsp, float sample_rate) {
  if (dsp)
    static_cast<AurivoDSP::MasterDSP *>(dsp)->setSampleRate(sample_rate);
}

void set_web_lpf(void *dsp, float freq) {
  if (dsp)
    static_cast<AurivoDSP::MasterDSP *>(dsp)->setWebLPF(freq);
}

void set_force_mute(void *dsp, int mute) {
  if (dsp)
    static_cast<AurivoDSP::MasterDSP *>(dsp)->setForceMute(mute != 0);
}

void process_monitor_audio(void *dsp, float *buffer, int numFrames,
                           int inSampleRate) {
  if (dsp)
    static_cast<AurivoDSP::MasterDSP *>(dsp)->processMonitorBuffer(
